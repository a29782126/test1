...
int main(int   argc,   char   *argv[])
{
	...
	{
		sprintf( DbgStr,  "MXEIO_E1K_Connect Failed Adapter IP=%s, TimeOut=%d, Password=%s\n",IPAddress, dwTimeOut, Password);
		MOXA_PRINTF( DbgStr);
	}
	...
	*/
	...
	if(wType == 0x1240)
	{
    	...
    	if(iRet == MXIO_OK)
    	{
			...
			{
				if(wValue[i] == 0)
    				sprintf( DbgStr, "ch[%d]=%s\n", (i + bytStartChannel), "0~10V");
				else if(wValue[i] == 1)
					sprintf( DbgStr, "ch[%d]=%s\n", (i + bytStartChannel), "4~20mA");
				else if(wValue[i] == 2)
					sprintf( DbgStr, "ch[%d]=%s\n", (i + bytStartChannel), "0~20mA");
				else if(wValue[i] == 4)
					sprintf( DbgStr, "ch[%d]=%s\n", (i + bytStartChannel), "4~20mA (Burn Out)");
				else
					sprintf( DbgStr, "ch[%d]=%s\n", (i + bytStartChannel), "Unknown");
				//
				MOXA_PRINTF( DbgStr);
			}
    	}
