...
int main(int argc, char *argv[])
{
    ...
    for(int i=0; i < DEVICE_COUNT; i++)
    {
        ...
        if(wType[i] == 0x1242)
        {
            WORD E1242_DI_VALUE = 0;
            bytStartChannel = 0;
            E1242_DI_VALUE += bytStartChannel;
            bytCount = 4;
            //==========================
            iRet = MXIO_ReadCoils( iHandle[i],          //the handle for a connection
                                   1,                   //coil type   1:read coils, 2:read discrete
                                   E1242_DI_VALUE,      //starting coil address
                                   bytCount,            //read coils count
                                   bytCoils );          //an array that stored the coil value

            CheckErr( iRet, (char*)"MXIO_ReadCoils" );
            if(iRet == MXIO_OK)
            {
                printf( "MXIO_ReadCoils succeed.\r\n");
                for(k=bytStartChannel; k < bytCount; k++ )
                {
                    printf( "DI%02d = %s\r\n", k, (bytCoils[0]&(1 << (k-bytStartChannel)))?"ON":"OFF");
                }
            }
        }
        //=========================================================================
        ...
        if(wType[i] == 0x1242)
        {
            WORD E1242_DO_VALUE = 0;
            bytStartChannel = 0;
            E1242_DO_VALUE += bytStartChannel;
            bytCount = 4;
            //==========================
            printf( "Set DO status mapping to DI channels.\r\n");
            iRet = MXIO_WriteCoils( iHandle[i],         //the handle for a connection
                                    E1242_DO_VALUE,     //starting coil address
                                    bytCount,           //write coils count
                                    bytCoils);          //wirte coils value

            CheckErr( iRet, (char*)"MXIO_WriteCoils" );
            if(iRet == MXIO_OK)
            {
                printf( "MXIO_WriteCoils succeeded.\r\n" );
                for(k=bytStartChannel; k < bytCount; k++)
                    printf( "Set DO%02d = %s\r\n", k, ((bytCoils[0] & (1 << (k-bytStartChannel))) > 0)?"ON":"OFF");
            }
            //==========================
        ...
        if(wType[i] == 0x1242)
        {
            WORD E1242_AI_VALUE = 520;
            bytStartChannel = 0;
            E1242_AI_VALUE += bytStartChannel;
            bytCount = 4;
            _ANALOG_VAL AnalogVal[4] = {0};
            WORD wRegister[8] = {0};
            //==========================
            iRet = MXIO_ReadRegs( iHandle[i],           //the handle for a connection
                                  4,                    //register type
                                                        //      3 : read holding registers
                                                        //      4 : read input register
                                  E1242_AI_VALUE,       //starting register address
                                  bytCount*2,           //read registers count
                                  wRegister );          //read registers value

            CheckErr( iRet, (char*)"MXIO_ReadRegs" );
            if(iRet == MXIO_OK)
            {
                printf( "MXIO_ReadRegs succeed.\r\n");
                for(k=bytStartChannel; k < bytCount; k++)
                {
					AnalogVal[k-bytStartChannel].iVal = wRegister[(k-bytStartChannel)*2+1];
					AnalogVal[k-bytStartChannel].iVal = AnalogVal[k-bytStartChannel].iVal << 16;
					AnalogVal[k-bytStartChannel].iVal |= wRegister[(k-bytStartChannel)*2];
					printf( "Get AI%02d = %.3f\r\n", k, AnalogVal[k-bytStartChannel].fVal);
                }
            }
            //==========================
            WORD E1242_AI_VALUE_RAW = 512;
            E1242_AI_VALUE_RAW += bytStartChannel;
            iRet = MXIO_ReadRegs( iHandle[i],           //the handle for a connection
                                  4,                    //register type
                                                        //      3 : read holding registers
                                                        //      4 : read input register
                                  E1242_AI_VALUE_RAW,   //starting register address
                                  bytCount,             //read registers count
                                  wRegister );          //read registers value
        ...
        if(wType[i] == 0x1260)
        {
            WORD E1260_RTD_VALUE = 1548;
            bytStartChannel = 0;
            E1260_RTD_VALUE += bytStartChannel;
            bytCount = 6;
            _ANALOG_VAL AnalogVal[6] = {0};
            WORD wRegister[12] = {0};
            //==========================
            iRet = MXIO_ReadRegs( iHandle[i],           //the handle for a connection
                                  4,                    //register type
                                                        //      3 : read holding registers
                                                        //      4 : read input register
                                  E1260_RTD_VALUE,      //starting register address
                                  bytCount*2,           //read registers count
                                  wRegister );          //read registers value
            ...
            //==========================
            WORD E1260_RTD_ENG_VALUE = 1536;
            E1260_RTD_ENG_VALUE += bytStartChannel;
            iRet = MXIO_ReadRegs( iHandle[i],           //the handle for a connection
                                  4,                    //register type
                                                        //      3 : read holding registers
                                                        //      4 : read input register
                                  E1260_RTD_ENG_VALUE,  //starting register address
                                  bytCount,             //read registers count
                                  wRegister );          //read registers value
